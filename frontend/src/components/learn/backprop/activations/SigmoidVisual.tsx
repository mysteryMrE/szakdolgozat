...
const SigmoidVisual = ({
  ...
  useEffect(() => {
    console.log("SigmoidVisual rendered");
  }, []);
  ...
  const pathD = useMemo(() => {
    ...
    return pts.join(" ");
  }, [rangeX, sigmoid, xToSvgCoord, yToSvgCoord]);

  const dotX = xToSvgCoord(x);
  const dotY = yToSvgCoord(sigmoid(x));
